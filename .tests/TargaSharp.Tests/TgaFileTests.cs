...
public class TgaFileTests
{
    ...
    public void Ctor_BitmapWithA1R5G5B5Palette_PreservesAlphaBitInColorMapData()
    {
        // Format8bppIndexed => IsColorMapped, and ColorMap2BytesEntry: true + a palette that mixes
        // opaque/transparent entries pushes LoadFunc(Bitmap) down the A1R5G5B5 write path (see bug #1:
        // the alpha bit was shifted into bit 22 instead of bit 15, so it was always lost on write since
        ...
        var tga = new TgaFile(bmp, ColorMap2BytesEntry: true);
